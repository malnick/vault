...
import (
	...
	pcfCertificates "github.com/hashicorp/vault-plugin-auth-pcf/testdata/certificate-generation"
	pcfAPI "github.com/hashicorp/vault-plugin-auth-pcf/testdata/pcf-api"
...
func TestPCFEndToEnd(t *testing.T) {
	...
	testPCFCerts, err := pcfCertificates.NewTestCerts(pcfAPI.FoundServiceGUID, pcfAPI.FoundOrgGUID, pcfAPI.FoundSpaceGUID, pcfAPI.FoundAppGUID, testIPAddress)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	os.Setenv(agentpcf.EnvVarInstanceCertificate, testPCFCerts.PathToInstanceCertificate)
	os.Setenv(agentpcf.EnvVarInstanceKey, testPCFCerts.PathToInstanceKey)
